...
(cl-defun make-powershell-commandstring (variable &key variable-definition)
  (cond
   ...
   ((eq variable-definition 'command-help)
    (format
     "get-command %s -syntax \n" variable))
   ))

(send-string (get-buffer-process "*powershell-babel*") "\"\n")

(defun ps|get-help ()
  (interactive)
  (message (let* ((psvariable (thing-at-point 'symbol t))
	 (proc (get-buffer-process ps-proc-buffer-name))
	 (parse-buffer ps-proc-buffer-name)
	 (powershell-string (make-powershell-commandstring psvariable :variable-definition 'command-help)))
    (with-current-buffer parse-buffer
      (erase-buffer));; clean old entries
    (send-string proc powershell-string)
